...
void binSort(list<studentRecord> &recordList, const int &dimension, const int &baseNumber) {
    ...
    for (int i = 0; i < dimension; i++)//最外层循环代表箱子排序的总次数
    {
        ...
        for (auto recordIterator(recordList.begin()); recordIterator != recordList.end(); recordIterator++)//第二层循环要将每一个number做基数拆解，根据这个拆解出来的数进行箱子排序
        {
...
int main() {
    ...
    studentRecord stu1(9, "小学生习近平");
    studentRecord stu2(45, "栗战书");
    studentRecord stu3(49, "王岐山");
    studentRecord stu4(80, "拜登");
    studentRecord stu5(32, "特鲁多");
    studentRecord stu6(76, "特朗普");
